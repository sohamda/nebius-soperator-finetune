#!/bin/bash
# =============================================================================
# Phi-3 Inference Comparison - Base vs Fine-tuned (Two Modes)
# =============================================================================
# Run 1: different-prompt (base=no sys prompt, finetuned=JSON sys prompt)
# Run 2: same-prompt (both get identical JSON sys prompt)
# Then combines results into a single report.
# =============================================================================

#SBATCH --job-name=phi3-compare
#SBATCH --output=/mnt/shared/logs/compare_%j.out
#SBATCH --error=/mnt/shared/logs/compare_%j.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
echo "Inference Comparison: Base vs Fine-tuned Phi-3 (both modes)"
echo "Start time: $(date)"
echo "======================================================================"

# Monitor GPU utilization in background
nvidia-smi dmon -s u -d 5 > /mnt/shared/logs/gpu_util_compare_${SLURM_JOB_ID}.log 2>&1 &
GPU_MON_PID=$!
...
python run_inference_compare.py \
    --checkpoint-dir /mnt/shared/checkpoints/phi3-json-mode \
    --use-4bit \
    --mode no-system \
    --output-tag nosys
...
with open('/mnt/shared/logs/comparison_nosys.json') as f:
    nosys = json.load(f)
with open('/mnt/shared/logs/comparison_withsys.json') as f:
    withsys = json.load(f)

print('=' * 70)
print('COMBINED COMPARISON RESULTS')
print('=' * 70)
print()
...
with open('/mnt/shared/logs/comparison_combined.json', 'w') as f:
    json.dump(combined, f, indent=2)
print(f'Combined results saved to /mnt/shared/logs/comparison_combined.json')
"
...
echo ""
echo "Output files:"
echo "  /mnt/shared/logs/comparison_nosys.json    (no system prompt)"
echo "  /mnt/shared/logs/comparison_withsys.json   (with JSON system prompt)"
echo "  /mnt/shared/logs/comparison_combined.json   (combined report)"
echo "  /mnt/shared/logs/gpu_util_compare_${SLURM_JOB_ID}.log"
